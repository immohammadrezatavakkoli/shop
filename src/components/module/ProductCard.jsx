...
const ProductCard = ({ title , price , discount , id }) => {
  ...
  return (
    <div className="rtl w-[14rem] h-80 flex flex-col justify-start items-center gap-1 bg-[#1A1A1A] rounded-md relative">
      {discount !== 0 && (
        <div className="w-[3rem] h-[3rem] flex flex-row justify-center items-center gap-[0.10rem] text-[#FBCB07] bg-[#242424] rounded-full absolute top-0 left-0 -mt-2 -ml-2 cursor-default border-4 border-[#FBCB07]">
      ...
      <Link href={`${pathParts}/${id}`} className="w-full h-3/6 flex flex-row justify-center items-center">
        ...
        <div className="w-full flex flex-row justify-between items-center px-1">
          <div className="h-full">
            <p className="flex flex-row items-center gap-1 text-[0.90rem] text-[#C3C4C5]">
              ...
              <span className="text-[0.70rem] text-[#C3C4C5]">
                تومان
              </span>
            </p>
          </div>
          <div className="h-full text-[#C3C4C5]">
            <button className="h-full text-[#FBCB07] text-[1.25rem]" onClick={AboutMessage}>
              <GoHeart />
            </button>
          </div>
        </div>
